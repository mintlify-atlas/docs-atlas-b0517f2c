---
title: "Window management in HagalazOS GUI"
...
Reads `btn.closest(".window-wrapper")` to find the target. Toggles between two states tracked by `win.dataset.max`:

| `dataset.max` | Result |
|---|---|
| `"0"` (normal) → `"1"` | `top:0`, `left:0`, `width:100%`, `height:calc(100% - 48px)` — fills the desktop above the taskbar |
| `"1"` (maximized) → `"0"` | Restores to `800px × 600px` at `top:50px / left:50px` |

`zIndexCounter` is incremented on every maximize/restore toggle.
...
`zIndexCounter` is a module-level integer starting at `1`. It increments in three situations:
...
3. **`mousedown` on any wrapper** — the wrapper that received the click gets a fresh counter value, bringing it visually to the front.
